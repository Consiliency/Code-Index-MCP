...
- **Tasks**:
  ...
  - verify: Live GitHub/tag/PyPI collision checks and exact release inputs. No effectful hosted preparation or publication.
...
`index_it_mcp.egg-info/**`. Only new PREP-created Qdrant proof outputs under
...
- Candidate PILOT driver prepare/offline/rehearsal/browser/verify-browser only; NO live mode.
...
- [ ] EC-PREP-1 - proven by `scripts/v13_release_candidate.py` and `.phase-loop/runs/v13-PREP-*/verification/verification.json`; falsified by metadata/runtime drift, failed installed construction-site controls or stale receipt bindings.
