...
owned client disposal, committed ignore-before-copy, bounded JSON/JSONL retention,
and async reindex recovery are implemented. Isolated test registries replace
legacy default-host access. Pure PILOT estimate: 79,360 upper-bound input units,
zero inference calls. Diagnostic wheel/container workflows pass; 151 public
branch/HTTP checks, 67 storage/export checks and 156 final fixture controls pass.
The broad diagnostic run's two incomplete mocks and the local gate's fake branch
switch were repaired. SL-2 records the candidate contract/runbook, but acceptance
still requires every original command on the final committed source. No DATA IF
or EC is accepted by this checkpoint; PILOT/PREP/SHIP have not begun.
...
2026-09-11 artifact integration checkpoint: 282 focused tests pass after
formatting. Production semantic rebuilds now share stable repository-relative
chunk identities and use stage-owned preflight; duplicate-content files and
repeat generations are covered. Incremental modify/rename/delete preserve
relational data and acknowledge collection-scoped cleanup. Verified artifact
restore merges retained local rows through the generation writer; portable
vector export streams mapped points instead of copying live storage. Restore
faults for missing/duplicate vectors, paths, dimensions, provenance and commit
leave old bytes intact and queries unavailable. SQLite export uses backup,
nested ignore filtering and relational cleanup; no excluded-path log.
The artifact coordinator and freshness consumers are explicitly owned by SL-1.

Still required: finish legacy artifact CLI/local-watcher caller alignment,
public query admission matrix (including the compatibility coordinator),
installed runtime fixtures, synthetic pilot estimate, full phase gates and
SL-2 evidence/docs. The 282 tests are checkpoint evidence only; no DATA IF/EC
is accepted and PILOT/PREP/SHIP have not begun.
